...
import numpy as np
from wise import DeepWise_PointWise_Conv

class LDFE(nn.Module):

    def __init__(self, inc, outc, kernel_size=3, padding=1,  stride=1, bias=None, lr_ratio=1.0):
        super(LDFE, self).__init__()
        ...
        self.conv_kernel = DeepWise_PointWise_Conv(inc, outc, stride=kernel_size)
        self.offset_conv = nn.Conv2d(inc, 2 * kernel_size * kernel_size, kernel_size=3, padding=1, stride=1)
        self.offset_conv.register_backward_hook(self._set_lr)
        self.lr_ratio = lr_ratio

    def _set_lr(self, module, grad_input, grad_output):#查看参数的梯度
