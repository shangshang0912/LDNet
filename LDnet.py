...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    return LDFE(in_planes, out_planes, kernel_size=3, stride=stride,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # residual function
        # self.residual_function = conv3x3(inplanes, planes, stride)
        self.residual_function = nn.Sequential(
            LDFE(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            LDFE(out_channels, out_channels * BasicBlock.expansion, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels * BasicBlock.expansion)
        )
        # expansion1 = 1
        self.ca = CCAM(in_channels=out_channels *BasicBlock.expansion, out_channels=out_channels * BasicBlock.expansion)
        # shortcut
        ...
        if stride != 1 or in_channels != BasicBlock.expansion * out_channels:
            self.shortcut = nn.Sequential(
                LDFE(in_channels, out_channels * BasicBlock.expansion, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(out_channels * BasicBlock.expansion)
            )
...
class ResNet(nn.Module):
    ...
    def __init__(self,in_channel,out_channel, block, num_block):
        ...
        self.conv1 = nn.Sequential(
            LDFE(in_channel, 64, kernel_size = 7, stride = 2, padding = 3,bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True))
